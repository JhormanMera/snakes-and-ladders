...
//-------------------------------------------------DICE AND PLAYER MOVES------------------------------------------------------------------------------------------------------------------------------
	...
	/** 
	 * Calculate the winner's score <b> 
	 * <b> pre: a player must have won the game </b> 
	 * <b> pre: the player 'temp' must to be the winner </b>
	 * <b> post: Calculate the winner's score </b>
	 */
	...
	/** 
	 * Randomly generate a number between 1 and 6 and then move the player to the corresponding node <b> 
	 * <b> pre: temp must be !=null </b> 
	 * <b> pre: the nodes must have been created previously </b>
	 * <b> post: the player has moved to the node indicating the randomly generated number </b>
	 * @return a message indicating the score obtained by the player after rolling the die and if so, to which node it will be moved if that node was the base of a ladder or the head of a snake
	 */
	...
	/** 
	 * Add the first player to the corresponding node and recursively call the 'setInBox' method <b> 
	 * <b> pre: the node in which the player is to be added must be !=null </b> 
	 * <b> pre: the player must have been created previously </b>
	 * <b> post: the first player was added to the corresponding node </b>
	...
	/** 
	 * Add the indicated player to the corresponding node <b> 
	 * <b> pre: the node in which the player is to be added must be !=null </b> 
	 * <b> pre: the player to be added must be 2nd or higher </b>
	 * <b> post: the player was added to the corresponding node </b>
	 * @param player type Player
	 * @param firstPlayer type Player
	 */
	...
	/** 
	 * Remove the indicated player from the corresponding node <b> 
	 * <b> pre: a player must have already been added to a specific node </b> 
	 * <b> post: the player has been removed from the node </b>
	 * @param node type Node
	 * @param player type Player
	 */
	 ...
	 * <b> post: the player switched positions with the next player </b>
	 */
...
//---------------------------------------------------------SNAKES--------------------------------------------------------------------------------------------------------------------
	...
	/** 
	 * The snake is created and head and tail nodes are assigned to it <b> 
	 * <b> pre: currentSnakes can't be > than half the rowsAmount of the game board </b> 
	 * <b> post: snakes were created and heads and tails were assigned </b>
	 * @param currentSnakes type int
	 * @param createdSnakes type int
	 */
	...
	/** 
	 * A node is put on the head of the snake and that snake is kept inside that node  <b> 
	 * <b> pre: the snake must have been created earlier </b> 
	 * <b> post: the node has been placed on the head of the snake and that snake is kept within that node </b>
	 * @param newSnake type Snake
	 */
	...
	/** 
	 * A node is put on the tail of the snake and that snake is kept inside that node  <b> 
	 * <b> pre: the snake must have been created earlier </b> 
	 * <b> post: the node has been placed on the tail of the snake and that snake is kept within that node </b>
	 * @param newSnake type Snake
	 */
...
//--------------------------------------------------------LADDERS--------------------------------------------------------------------------------------------------------------------------------------------
	...
	/** 
	 * The ladder is created and base and top nodes are assigned to it <b> 
	 * <b> pre: currentLadders can't be > than half the rowsAmount of the game board </b> 
	 * <b> post: ladders were created and bases and tops were assigned </b>
	 * @param currentLadders type int
	 * @param createdLadders type int
	 */
	...
	/** 
	 * A node is put on the base of the ladder and that ladder is kept inside that node  <b> 
	 * <b> pre: the ladder must have been created earlier </b> 
	 * <b> post: the node has been placed on the base of the ladder and that ladder is kept within that node </b>
	 * @param newLadder type Ladder
	 */
	...
	/** 
	 * A node is put on the top of the ladder and that ladder is kept inside that node  <b> 
	 * <b> pre: the ladder must have been created earlier </b> 
	 * <b> post: the node has been placed on the top of the ladder and that ladder is kept within that node </b>
	 * @param newLadder type Ladder
	 */
...
//----------------------------------------------------BINARY SEARCH TREE---------------------------------------------------------------------------------------------	
	 ...
	 * <b> pre: the player that is passed as a parameter must be !null </b> 
	 * <b> post: adds the first winner to the binary search tree and adds the other winners with recursive call </b>
	 * @param player type Player
	 */
	...
	/** 
	 * Adds the other winners to the binary search tree <b> 
	 * <b> pre: the player that is passed as a parameter must be !null </b> 
	 * <b> post: adds the other winners to the binary search tree </b>
	 * @param current type Player
	 * @param newWinner type Player
	 */
	...
	/** 
	 * Calls recursive method 'printWinners' <b> 
	 * <b> post: shows who the winners have been and also shows that there aren't winners yet </b>
	 */
	...
	/** 
	 * Print a string showing the winners and if there are no winners yet, print a string saying there are no winners yet <b> 
	 * <b> pre: the player that is passed as a parameter must be !=null </b> 
	 * <b> post: shows who the winners have been and also shows that there aren't winners yet </b>
	 * @param player type Player
	 */
